...
class MSELoss:
    def forward(self, A, Y):
        ...
        self.A = A
        self.Y = Y
        self.N = np.shape(self.A)[0]  # TODO
        self.C = np.shape(A)[1] # TODO
        se = (self.A - self.Y) * (self.A - self.Y) # TODO
        sse = np.ones(shape=(1,self.N)) @ se @ np.ones(shape=(self.C,1)) # TODO
...
class CrossEntropyLoss:
    def forward(self, A, Y):
        ...
        self.Y = Y
        self.N = np.shape(self.A)[0]  # TODO
        self.C = np.shape(self.A)[1]  # TODO

        Ones_C = np.ones(shape=(self.C,1))  # TODO
        Ones_N = np.ones(shape=(self.N,1))  # TODO
